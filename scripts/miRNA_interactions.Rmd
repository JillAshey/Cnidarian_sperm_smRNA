...
join_interaction <- rbind(miranda_apoc_interaction, merged_3UTR_apoc_interaction)
```

Plot number of binding bp. 
```{r}
ggplot(join_interaction, aes(x = interaction_length)) +
  geom_histogram(
    aes(y = after_stat(count / sum(count))),
    binwidth = 1, fill = "steelblue", color = "black"
  ) +
...
# calculate percentages per bin
df_freq_3UTR_apoc <- join_interaction %>%
  count(interaction_length) %>%
  mutate(perc = n / sum(n) * 100)
...
join_interaction <- rbind(miranda_nvec_interaction, merged_3UTR_nvec_interaction)
```
...
# calculate percentages per bin
df_freq_3UTR_nvec <- join_interaction %>%
  count(interaction_length) %>%
  mutate(perc = n / sum(n) * 100)
...
join_interaction <- rbind(miranda_ahya_interaction, merged_3UTR_ahya_interaction)
```
...
# calculate percentages per bin
df_freq_3UTR_ahya <- join_interaction %>%
  count(interaction_length) %>%
  mutate(perc = n / sum(n) * 100)

ggplot(df_freq_3UTR_ahya, aes(x = interaction_length, y = perc)) +
  # filled frequency curve
  geom_area(fill = "steelblue", alpha = 0.6) +
  geom_line(color = "steelblue", size = 1.2) +
  ...
  # axis breaks
  scale_x_continuous(breaks = seq(min(df_freq_3UTR_ahya$interaction_length),
                                  max(df_freq_3UTR_ahya$interaction_length), by = 2)) +
  scale_y_continuous(breaks = seq(0, max(df_freq_3UTR_ahya$perc), by = 5)) +
