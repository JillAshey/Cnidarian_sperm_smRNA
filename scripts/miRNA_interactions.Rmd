...
# Count number of annotations with GO terms
sum((!is.na(annot_apoc$GO_Swiss.Prot) & annot_apoc$GO_Swiss.Prot != "") | (!is.na(annot_apoc$GO_Trembl)     & annot_apoc$GO_Trembl!= ""))
```
...
Make list of genes with binding targets in mRNA and 3'UTR regions. 
```{r}
# Genes of interest - ie those targeted by miRNAs 
apoc_clust_genes <- as.character(unique(c(miranda_apoc$mRNA, merged_3UTR_apoc$gene_name)))
...
Create `topGOdata` object, which is required for topGO analysis
```{r}
GO_APOC_BOTH_BP <-new("topGOdata", ontology="BP", gene2GO=apoc_gene2go_topgo, allGenes=apoc_GeneList, annot = annFUN.gene2GO, geneSel=topDiffGenes)
```

Run GO enrichment test 
```{r}
GO_APOC_BOTH_BP_FE <- runTest(GO_APOC_BOTH_BP, algorithm="weight01", statistic="fisher")
```

Generate results table 
```{r}
GO_APOC_BOTH_BP_En <- GenTable(GO_APOC_BOTH_BP, Fisher = GO_APOC_BOTH_BP_FE, orderBy = "Fisher", numChar = 51, topNodes = 100)
```
Only taking the top 100 GO terms 

Filter by significant results
```{r}
GO_APOC_BOTH_BP_En$Fisher<-as.numeric(GO_APOC_BOTH_BP_En$Fisher)
GO_APOC_BOTH_BP_En_sig<-GO_APOC_BOTH_BP_En[GO_APOC_BOTH_BP_En$Fisher<0.05,]
```
...
Create `topGOdata` object, which is required for topGO analysis
```{r}
GO_APOC_BOTH_MF <-new("topGOdata", ontology="MF", gene2GO=apoc_gene2go_topgo, allGenes=apoc_GeneList, annot = annFUN.gene2GO, geneSel=topDiffGenes)
```

Run GO enrichment test 
```{r}
GO_APOC_BOTH_MF_FE <- runTest(GO_APOC_BOTH_MF, algorithm="weight01", statistic="fisher")
```

Generate results table 
```{r}
GO_APOC_BOTH_MF_En <- GenTable(GO_APOC_BOTH_MF, Fisher = GO_APOC_BOTH_MF_FE, orderBy = "Fisher", numChar = 51, topNodes = 100)
```
Only taking the top 100 GO terms 

Filter by significant results
```{r}
GO_APOC_BOTH_MF_En$Fisher<-as.numeric(GO_APOC_BOTH_MF_En$Fisher)
GO_APOC_BOTH_MF_En_sig<-GO_APOC_BOTH_MF_En[GO_APOC_BOTH_MF_En$Fisher<0.05,]
```

##### Plotting BP + MF terms for both bindings

Plot top 10 terms
```{r}
## Bind dfs 
GO_APOC_BOTH_BP_En_sig$ontology <- "Biological Processes"
GO_APOC_BOTH_MF_En_sig$ontology <- "Molecular Functions"
BP_MF_BOTH_APOC <- rbind(GO_APOC_BOTH_BP_En_sig, GO_APOC_BOTH_MF_En_sig)

## Select top 10 terms for each ontology 
top10_per_ontology <- BP_MF_BOTH_APOC %>%
  group_by(ontology) %>%
  slice_max(Significant, n = 10, with_ties = FALSE) %>%
  ungroup()
...
annot_nvec |>
  dplyr::filter(!is.na(GeneOntologyIDs) & GeneOntologyIDs != "") |>
...
```{r}
# Genes of interest - ie those targeted by miRNAs 
nvec_clust_genes <- as.character(unique(c(miranda_nvec$mRNA, merged_3UTR_nvec$gene_name)))
...
Create `topGOdata` object, which is required for topGO analysis
```{r}
GO_NVEC_BOTH_BP <-new("topGOdata", ontology="BP", gene2GO=nvec_gene2go_topgo, allGenes=nvec_GeneList, annot = annFUN.gene2GO, geneSel=topDiffGenes)
```

Run GO enrichment test 
```{r}
GO_NVEC_BOTH_BP_FE <- runTest(GO_NVEC_BOTH_BP, algorithm="weight01", statistic="fisher")
```

Generate results table 
```{r}
GO_NVEC_BOTH_BP_En <- GenTable(GO_NVEC_BOTH_BP, Fisher = GO_NVEC_BOTH_BP_FE, orderBy = "Fisher", numChar = 51, topNodes = 100)
```
Only taking the top 100 GO terms 

Filter by significant results
```{r}
GO_NVEC_BOTH_BP_En$Fisher<-as.numeric(GO_NVEC_BOTH_BP_En$Fisher)
GO_NVEC_BOTH_BP_En_sig<-GO_NVEC_BOTH_BP_En[GO_NVEC_BOTH_BP_En$Fisher<0.05,]
```
...
Create `topGOdata` object, which is required for topGO analysis
```{r}
GO_NVEC_BOTH_MF <-new("topGOdata", ontology="MF", gene2GO=nvec_gene2go_topgo, allGenes=nvec_GeneList, annot = annFUN.gene2GO, geneSel=topDiffGenes)
```

Run GO enrichment test 
```{r}
GO_NVEC_BOTH_MF_FE <- runTest(GO_NVEC_BOTH_MF, algorithm="weight01", statistic="fisher")
```

Generate results table 
```{r}
GO_NVEC_BOTH_MF_En <- GenTable(GO_NVEC_BOTH_MF, Fisher = GO_NVEC_BOTH_MF_FE, orderBy = "Fisher", numChar = 51, topNodes = 100)
```
Only taking the top 100 GO terms 

Filter by significant results
```{r}
GO_NVEC_BOTH_MF_En$Fisher<-as.numeric(GO_NVEC_BOTH_MF_En$Fisher)
GO_NVEC_BOTH_MF_En_sig<-GO_NVEC_BOTH_MF_En[GO_NVEC_BOTH_MF_En$Fisher<0.05,]
```
...
```{r}
## Bind dfs 
GO_NVEC_BOTH_BP_En_sig$ontology <- "Biological Processes"
GO_NVEC_BOTH_MF_En_sig$ontology <- "Molecular Functions"
BP_MF_BOTH_NVEC <- rbind(GO_NVEC_BOTH_BP_En_sig, GO_NVEC_BOTH_MF_En_sig)

## Select top 10 terms for each ontology 
top10_per_ontology <- BP_MF_BOTH_NVEC %>%
  group_by(ontology) %>%
  slice_max(Significant, n = 10, with_ties = FALSE) %>%
  ungroup()
...
# Count number of annotations with GO terms
sum(!is.na(annot_ahya$GO.Terms) & annot_ahya$GO.Terms != "")
```
...
DO BINDING AND ENRICHMENT METRICS 
...
```{r}
# Genes of interest - ie those targeted by miRNAs 
ahya_clust_genes <- as.character(unique(c(miranda_ahya$mRNA, merged_3UTR_ahya$gene_name)))
...
Create `topGOdata` object, which is required for topGO analysis
```{r}
GO_AHYA_BOTH_BP <-new("topGOdata", ontology="BP", gene2GO=ahya_gene2go_topgo, allGenes=ahya_GeneList, annot = annFUN.gene2GO, geneSel=topDiffGenes)
```

Run GO enrichment test 
```{r}
GO_AHYA_BOTH_BP_FE <- runTest(GO_AHYA_BOTH_BP, algorithm="weight01", statistic="fisher")
```

Generate results table 
```{r}
GO_AHYA_BOTH_BP_En <- GenTable(GO_AHYA_BOTH_BP, Fisher = GO_AHYA_BOTH_BP_FE, orderBy = "Fisher", numChar = 51, topNodes = 100)
```
Only taking the top 100 GO terms 

Filter by significant results
```{r}
GO_AHYA_BOTH_BP_En$Fisher<-as.numeric(GO_AHYA_BOTH_BP_En$Fisher)
GO_AHYA_BOTH_BP_En_sig<-GO_AHYA_BOTH_BP_En[GO_AHYA_BOTH_BP_En$Fisher<0.05,]
```
...
Create `topGOdata` object, which is required for topGO analysis
```{r}
GO_AHYA_BOTH_MF <-new("topGOdata", ontology="MF", gene2GO=ahya_gene2go_topgo, allGenes=ahya_GeneList, annot = annFUN.gene2GO, geneSel=topDiffGenes)
```

Run GO enrichment test 
```{r}
GO_AHYA_BOTH_MF_FE <- runTest(GO_AHYA_BOTH_MF, algorithm="weight01", statistic="fisher")
```

Generate results table 
```{r}
GO_AHYA_BOTH_MF_En <- GenTable(GO_AHYA_BOTH_MF, Fisher = GO_AHYA_BOTH_MF_FE, orderBy = "Fisher", numChar = 51, topNodes = 100)
```
Only taking the top 100 GO terms 

Filter by significant results
```{r}
GO_AHYA_BOTH_MF_En$Fisher<-as.numeric(GO_AHYA_BOTH_MF_En$Fisher)
GO_AHYA_BOTH_MF_En_sig<-GO_AHYA_BOTH_MF_En[GO_AHYA_BOTH_MF_En$Fisher<0.05,]
```
...
```{r}
## Bind dfs 
GO_AHYA_BOTH_BP_En_sig$ontology <- "Biological Processes"
GO_AHYA_BOTH_MF_En_sig$ontology <- "Molecular Functions"
BP_MF_BOTH_AHYA <- rbind(GO_AHYA_BOTH_BP_En_sig, GO_AHYA_BOTH_MF_En_sig)

## Select top 10 terms for each ontology 
top10_per_ontology <- BP_MF_BOTH_AHYA %>%
  group_by(ontology) %>%
  slice_max(Significant, n = 10, with_ties = FALSE) %>%
  ungroup()
