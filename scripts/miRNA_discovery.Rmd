...
Save identified miRNAs. 
```{r}
write.csv(mirna_filtered_apoc, "../output/shortstack/apoc/trimmed_default/apoc_miRNAs.csv")
```
...
Save identified miRNAs. 
```{r}
write.csv(mirna_filtered_nvec, "../output/shortstack/nvec/trimmed_default/nvec_miRNAs.csv")
```
...
Remove `efu-miR-9341`, `bta-miR-2478`, and `mse-miR-2779;ipu-miR-7550;oni-miR-7550`-- they are mapping multiple times across the genome and corresponds to bat and mouse miRNAs.
```{r}
mirna_filtered_ahya <- mirna_filtered_ahya %>%
  filter(!known_miRNAs == "efu-miR-9341") %>%
  filter(!known_miRNAs == "bta-miR-2478") %>%
  filter(!known_miRNAs == "mse-miR-2779;ipu-miR-7550;oni-miR-7550")
...
mirna_list_counts_ahya <- counts_mirna_filt_ahya$Name
```

We are left with 25 miRNAs expressed. 

Filter Ahya miRNA info by the remaining expressed cluster names 
```{r}
mirna_filtered_ahya <- mirna_filtered_ahya[mirna_filtered_ahya$Name %in% mirna_list_counts_ahya, ]
``` 

Save identified miRNAs. 
```{r}
write.csv(mirna_filtered_ahya, "../output/shortstack/ahya/trimmed_default/ahya_miRNAs.csv")
```
