...
Only keep seqs that have length between 20-26
```{r}
...
Load data
```{r}
mirna_trim_ahya <- read.table("../output/shortstack/ahya/trimmed_default/Results.txt", header = T)
```

Select data with miRNA == Y or known miRNA
```{r}
mirna_trim_filtered_ahya <- mirna_trim_ahya %>%
  filter(MIRNA == "Y" | !is.na(known_miRNAs)) %>%
 mutate(Length = End - Start) %>%
 mutate(mature_length = nchar(MajorRNA))
```
